import { decode, BN, getAddress, hexZeroPad, hexlify, keccak256, encode, recoverAddress, sign, AvalancheAddress, arrayify, stripZeros, splitSignature, getContractAddress } from 'avalanche-js-crypto';
import _regeneratorRuntime from 'regenerator-runtime';
import { isHex, hexToNumber, isAddress, strip0x, ChainType, add0xToString, Unit, numberToHex, AddressSuffix } from 'avalanche-js-utils';
import { Messenger, HttpProvider, NewHeaders, Emitter, RPCMethod } from 'avalanche-js-network';
...
var recover = function recover(rawTransaction) {
  ...
  var tx = {
    ...
    to: handleAddress(transaction[5]) !== '0x' ? getAddress(handleAddress(transaction[5])).checksum : '0x',
  ...
  if (new BN(strip0x(handleNumber(tx.signature.r))).isZero() && new BN(strip0x(handleNumber(tx.signature.s))).isZero()) {
...
var recoverETH = function recoverETH(rawTransaction) {
  ...
  var tx = {
    ...
    shardID: 0,
    toShardID: 0,
    to: handleAddress(transaction[3]) !== '0x' ? getAddress(handleAddress(transaction[3])).checksum : '0x',
...
var defaultMessenger = /*#__PURE__*/new Messenger( /*#__PURE__*/new HttpProvider('http://localhost:9500'), ChainType.Avalanche);
...
var TransactionBase = /*#__PURE__*/function () {
  function TransactionBase(messenger, txStatus) {
    ...
    this.cxStatus = TxStatus.INTIALIZED;
    this.cxBlockNumbers = [];
  ...
  TransactionBase.normalizeAddress = function normalizeAddress(address) {
    if (address === '0x') {
      return '0x';
    } else if (AvalancheAddress.isValidChecksum(address) || AvalancheAddress.isValidBech32(address) || AvalancheAddress.isValidBech32TestNet(address)) {
      ...
      return _regeneratorRuntime.wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            ...
            case 2:
              _context.next = 4;
              return this.messenger.send(RPCMethod.GetTransactionReceipt, txHash, this.messenger.chainType, typeof shardID === 'string' ? Number.parseInt(shardID, 10) : shardID);
              ...
              while (1) {
                switch (_context3.prev = _context3.next) {
                  ...
                  case 12:
                    ...
                    _this.txStatus = TxStatus.REJECTED;
        ...
        while (1) {
          switch (_context5.prev = _context5.next) {
            case 0:
              _context5.prev = 0;
              _context5.next = 3;
              return this.messenger.send(RPCMethod.BlockNumber, [], this.messenger.chainPrefix, typeof shardID === 'string' ? Number.parseInt(shardID, 10) : shardID);
            ...
            case 6:
              return _context5.abrupt("return", new BN(currentBlock.result.replace('0x', ''), 'hex'));
        ...
        while (1) {
          switch (_context6.prev = _context6.next) {
            case 0:
              _context6.prev = 0;
              _context6.next = 3;
              return this.messenger.send(RPCMethod.GetBlockByNumber, [blockNumber, true], this.messenger.chainPrefix, typeof this.shardID === 'string' ? Number.parseInt(this.shardID, 10) : this.shardID);
      ...
      return _regeneratorRuntime.wrap(function _callee8$(_context8) {
        while (1) {
          switch (_context8.prev = _context8.next) {
            ...
            case 2:
              _context8.next = 4;
              return this.messenger.send(RPCMethod.GetCXReceiptByHash, txHash, this.messenger.chainPrefix, typeof toShardID === 'string' ? Number.parseInt(toShardID, 10) : toShardID);
  ...
  _proto.socketCxConfirm = function socketCxConfirm(txHash, maxAttempts, toShardID) {
    ...
    return new Promise(function (resolve, reject) {
      var newHeads = Promise.resolve(new NewHeaders(_this2.messenger, typeof toShardID === 'string' ? Number.parseInt(toShardID, 10) : toShardID));
      newHeads.then(function (p) {
        p.onData( /*#__PURE__*/function () {
              ...
              while (1) {
                switch (_context9.prev = _context9.next) {
                  ...
                  case 12:
                    ...
                    _this2.cxStatus = TxStatus.REJECTED;
...
var Transaction = /*#__PURE__*/function (_TransactionBase) {
  ...
  function Transaction(params, messenger, txStatus) {
    ...
    _this.gasPrice = params && params.gasPrice ? new Unit(params.gasPrice).asWei().toWei() : new Unit(0).asWei().toWei();
    _this.gasLimit = params && params.gasLimit ? new Unit(params.gasLimit).asWei().toWei() : new Unit(0).asWei().toWei();
    ...
    _this.value = params && params.value ? new Unit(params.value).asWei().toWei() : new Unit(0).asWei().toWei();
    ...
    _this.cxStatus = _this.isCrossShard() ? TxStatus.INTIALIZED : TxStatus.NONE;
    return _this;
  }
  /**
   *
  ...
  _proto.getRLPUnsigned = function getRLPUnsigned() {
    ...
    var fields = this.messenger.chainType === ChainType.Avalanche ? transactionFields : transactionFieldsETH;
    fields.forEach(function (field) {
      var value = _this2.txParams[field.name] || [];
      value = arrayify(hexlify(field.transform === 'hex' ? add0xToString(value.toString(16)) : value)); // Fixed-width field
  ...
  _proto.getRLPSigned = function getRLPSigned(raw, signature) {
    ...
    var rawLength = this.messenger.chainType === ChainType.Avalanche ? 11 : 9;
  ...
  _proto.recover = function recover$1(rawTransaction) {
    // temp setting to be compatible with eth
    var recovered = this.messenger.chainType === ChainType.Avalanche ? recover(rawTransaction) : recoverETH(rawTransaction);
  ...
  _proto.setParams = function setParams(params) {
    ...
    this.gasPrice = params && params.gasPrice ? new Unit(params.gasPrice).asWei().toWei() : new Unit(0).asWei().toWei();
    this.gasLimit = params && params.gasLimit ? new Unit(params.gasLimit).asWei().toWei() : new Unit(0).asWei().toWei();
    ...
    this.value = params && params.value ? new Unit(params.value).asWei().toWei() : new Unit(0).asWei().toWei();
      ...
      return _regeneratorRuntime.wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            ...
            case 4:
              _context.next = 6;
              return this.messenger.send(RPCMethod.SendRawTransaction, this.rawTransaction, this.messenger.chainType, typeof this.shardID === 'string' ? Number.parseInt(this.shardID, 10) : this.shardID);
            ...
            case 6:
              ...
              this.setTxStatus(TxStatus.PENDING); // await this.confirm(this.id, 20, 1000);
...
var TransactionFactory = /*#__PURE__*/function () {
  ...
  TransactionFactory.getContractAddress = function getContractAddress$1(tx) {
  ...
  _proto.recover = function recover(txHash) {
    var newTxn = new Transaction({}, this.messenger, TxStatus.INTIALIZED);
...
export { AbstractTransaction, RLPSign, ShardingTransaction, Transaction, TransactionBase, TransactionEvents, TransactionFactory, TxStatus, defaultMessenger, handleAddress, handleNumber, recover, recoverETH, sleep, transactionFields, transactionFieldsETH };
//# sourceMappingURL=avalanche-js-transaction.esm.js.map
